...
class Utils:
    '''Class containing utility functions for the Streamlit-LSTM-Rainfall project.'''

    def __init__(self, locations, format, start_date_str, end_date_str):
        # The list below includes objects that need to be exposed when calling this class
        # Add function arguments to this class only if they need to be passed around when functions call each other
        self.values = self.read_data_from_pickles(locations)
        self.coordinates = self.read_coordinates()
        self.selected_date = self.add_time_slider(format, start_date_str, end_date_str)
    ...
    def read_data_from_pickles(locations):
        ...
        with open('./results/evaluation/eval.pkl', 'rb') as f:
            data = pickle.load(f)
    ...
    def read_coordinates():
        ...
        base_file = pd.read_csv('./data/LOCATIONS.csv')
